...
geographic_area_sentiment_analysis_prompt = """
    ...
    Let the features be labeled as title, classification, summary, and reasoning. 
    """
bot_detection_sentiment_analysis_prompt = """
    Perform sentiment analysis on a piece of provided text, looking to classify if the text was written by Bot.
    Provide following in JSON format: an appropriate title of the article, the classification of the text if it written by Bot or not, 
    a summary of the article which is at most 3 sentences, and the reasoning for the classification which is at most three sentences. 
    Let the features be labeled as title, classification, summary, and reasoning. 
    """
...
prompt_map = {
              ...
              "Bot Detection": bot_detection_sentiment_analysis_prompt,
             }
    
analysis_types = ["Positive/Negative", "Political", "Sarcasm"]
...
def read_pdf_file(file_name):
    with fitz.open('./Data/' + file_name) as doc:  # open document
...
def run_analysis_web(text, analysis_type):   
    prompt = ""
